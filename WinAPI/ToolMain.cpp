...
HRESULT ToolMain::Init()
{
	...
	img = IMAGEMANAGER->findImage("tile_main");
...
void ToolMain::Update()
{
	...
	if(_ptMouse.x < WINSIZE_X / 8 && startX > 0
		&& SUBWIN->GetIsActive() == false) {
		startX--;
		endX--;
	}
	//if (INPUT->GetKey('D') && endX < MAXWINTILEX * 2) {
	if (_ptMouse.x > WINSIZE_X - WINSIZE_X / 8 && endX < MAXWINTILEX * 2
		&& SUBWIN->GetIsActive() == false) {
		startX++;
		endX++;
	}
	//if (INPUT->GetKey('W') && startY > 0) {
	if (_ptMouse.y < WINSIZE_Y / 8 && startY > 0
		&& SUBWIN->GetIsActive() == false) {
		startY--;
		endY--;
	}
	//if (INPUT->GetKey('S') && endY < MAXWINTILEY * 2) {
	if (_ptMouse.y > WINSIZEY - WINSIZEY / 8 && endY < MAXWINTILEY * 2
		&& SUBWIN->GetIsActive() == false) {
		startY++;
		endY++;
	}
